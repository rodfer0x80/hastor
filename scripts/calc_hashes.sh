...
# Function to convert a string to its BEncode representation
to_bencode() {
  local input="$1"
  local type="$2" # "string", "integer", or "dict", "list"
...
# Function to calculate the SHA1 hash of a string and convert to hex
sha1_hash() {
  local input="$1"
  # Use openssl for SHA1 hashing and convert to hex in one step.
  echo -n "$input" | openssl dgst -sha1 | sed 's/^.* //g'
}

# Function to calculate the BEncode hash as in the Haskell code
calculate_bencode_hash() {
  local bencode_input="$1"
  local encoded_bencode
  local raw_hash

  # Encode the input string as BEncode.
  encoded_bencode=$(to_bencode "$bencode_input" "dict")

  # Calculate the SHA1 hash of the BEncode encoding.
